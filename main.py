...
# ---------------------------------------------------------------------------
# Lightweight TFBoard toggle:  set  DISABLE_TFBOARD=1  in the environment
# (or in train_pcn.sh) to skip tensorboard logging and save disk space.
# ---------------------------------------------------------------------------
DISABLE_TFBOARD = os.environ.get("DISABLE_TFBOARD", "0") == "1"

if not DISABLE_TFBOARD:
    try:
        from tensorboardX import SummaryWriter
    except ImportError:
        DISABLE_TFBOARD = True
...
def create_experiment_dir(args):
    ...
    if not DISABLE_TFBOARD and not os.path.exists(args.tfboard_path):
...
def main(args):
    ...
    # define the tensorboard writer (disabled when DISABLE_TFBOARD=1)
    train_writer = None
    val_writer = None
    if not args.test:
        if not DISABLE_TFBOARD and args.local_rank == 0:
    ...
    if DISABLE_TFBOARD:
        logger.info('TensorBoard logging DISABLED (DISABLE_TFBOARD=1)')
    # set random seeds
