...
class PCN(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Resample gt to exactly self.npoints if needed
        ngt = data['gt'].shape[0]
        if ngt != self.npoints:
            if ngt > self.npoints:
                choice = np.random.permutation(ngt)[:self.npoints]
                data['gt'] = data['gt'][choice]
            else:
                # pad by repeating existing points
                shortage = self.npoints - ngt
                choice = np.random.choice(ngt, shortage, replace=True)
                data['gt'] = np.concatenate([data['gt'], data['gt'][choice]], axis=0)
